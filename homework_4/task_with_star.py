...
def max_division_by_3(num):
    """функция нахождения наибольшего числа по условиям:
    ...
    :return: int(new_num) - наибольшее число, удовлетворяющее необходимым условиям
    """

    new_num = ''
    for i in range(len(str(num))):
        for j in range(9, 1, -1):
            new_num = str(num)[:i] + str(j) + str(num)[i + 1:]
            if int(new_num) % 3 == 0:
                break
        if (int(new_num) % 3 == 0) and (int(new_num) > num):
            break
